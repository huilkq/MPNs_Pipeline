...
from MPNs_PURE.shared.data_structures import Dataset, Pre_data
...
def generate_pre_relation_data(entity_data, use_gold=False, context_window=0):
    """
    Prepare data for the relation model
    If training: set use_gold = True
    """
    logger.info('Generate relation data from %s' % (entity_data))
    data = Pre_data(entity_data)
