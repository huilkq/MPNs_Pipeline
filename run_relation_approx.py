...
def get_features_from_file(filename, label2id, max_seq_length, tokenizer, special_tokens, use_gold, context_window,
    ...
    for doc in data:
        for i, sent in enumerate(doc):
            ...
            assert (num_tokens + 3 <= max_seq_length)
            ...
            for x in range(len(sent_ner)):
                for y in range(len(sent_ner)):
                    ...
                    position_ids = position_ids + [token_start[sent_start + sub.span.start_sent],
...
def evaluate(model, device, eval_dataloader, e2e_ngold=None, verbose=True):
    ...
    for input_ids, input_position, input_mask, segment_ids, labels, sub_obj_ids, sub_obj_masks in eval_dataloader:
        ...
        with torch.no_grad():
            logits = model(input_ids, segment_ids, input_mask, labels=None, sub_obj_ids=sub_obj_ids, sub_obj_masks=None,
...
def main(args):
    ...
    if args.do_eval and (args.do_train or not (args.eval_test)):
        eval_dataset, eval_features, eval_nrel = get_features_from_file(
            os.path.join(args.entity_output_dir, args.entity_predictions_dev), label2id, args.max_seq_length, tokenizer,
            special_tokens, use_gold=args.eval_with_gold, context_window=args.context_window,
            batch_computation=args.batch_computation, unused_tokens=not (args.add_new_tokens))
        logger.info("***** Dev *****")
    ...
    if args.do_train:
        train_dataset, train_features, train_nrel = get_features_from_file(
            args.train_file, label2id, args.max_seq_length, tokenizer, special_tokens, use_gold=True,
        ...
        train_data = TensorDataset(all_input_ids, all_position_ids, all_input_mask, all_segment_ids, all_labels,
                                   all_sub_obj_ids, all_sub_obj_masks)
        ...
        for epoch in range(int(args.num_train_epochs)):
            ...
            for step, batch in enumerate(train_batches):
                ...
                loss = model(input_ids, segment_ids, input_mask, labels=labels, sub_obj_ids=sub_obj_ids,
                             sub_obj_masks=sub_obj_masks, input_position=input_position)
                if n_gpu > 1:
                    loss = loss.mean()
    ...
    if args.do_eval:
        if args.eval_test:
            eval_dataset, eval_features, eval_nrel = get_features_from_file(
                os.path.join(args.entity_output_dir, args.entity_predictions_test), label2id, args.max_seq_length,
                tokenizer, special_tokens, use_gold=args.eval_with_gold, context_window=args.context_window,
                batch_computation=args.batch_computation, unused_tokens=not (args.add_new_tokens))
            logger.info("***** Test *****")
            logger.info("  Num examples = %d", len(eval_features))
            logger.info("  Batch size = %d", args.eval_batch_size)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--model", default='D:\pythonProject\BIONER_ER\pre_models\matscibert', type=str)
    ...
    parser.add_argument("--train_file", default='D:\pythonProject\MPNs_PURE\json\\output.json', type=str,
    ...
    parser.add_argument("--entity_output_dir", type=str, default='D:\pythonProject\MPNs_PURE\entity_output',
